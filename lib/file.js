...
exports.moveFile = function(args1, args2) {
    fs.readFile(path_todo, 'utf8', function (err, data) {
      ...
      var lines = data.split(/\n/);
      if(args1 < 0 || args1 > lines.length || args2 < 0 || args2 > lines.length || args1 === args2) {
      ...
      fs.unlink(path_todo, function (err) {
        if (err) {
          ...
          console.log(chalk.green('to do successfully moved'));
        }
        if(args1 < args2) {
          for(let i = 0 ; lines.length - 1 >= i ; i++) {
              if(i !== lines.length - 1) {
          ...
          for(let i = 0 ; lines.length - 1 >= i ; i++) {
              ...
              if(i === args1) {
                fs.appendFile(path_todo, lines[args2], function (err) {
                  if (err) {
                      throw err;
                  }
                });
